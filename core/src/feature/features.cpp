...
void features::to_vw(Eigen::SparseVector<float> **vw_features) {
    ...
    *vw_features = new Eigen::SparseVector<float>(feature_hashing::size());

    for_each(before_hashing.begin(), before_hashing.end(),
             [&vw_features](optional<tuple<string, string, float>> feature) {
                 if (feature.is_initialized()) {
                     tuple<string, string, float> &f = feature.get();
                     auto &ns = std::get<0>(f);
                     auto &name = std::get<1>(f);
                     auto &value = std::get<2>(f);
                     auto hash_code = feature_hashing::hash_single_feature(ns, name);
